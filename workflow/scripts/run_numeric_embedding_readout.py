...
from numeric_embedding_readout import (ROOT, RESULT, CROPS, CONDITIONS, HEADS, DIMS,
    run_root, load, load_bins, NeuralReadout, predict, versions)
...
CODE = ('run_numeric_embedding_readout.py', 'numeric_embedding_readout.py',
...
def main():
    ...
    with (root / 'run.lock').open('a') as lock:
        ...
        spec = dict(**vars(args), seed=42, input_manifest=meta, versions=versions(),
            ...
            inputs='Original W18 inputs with training-only quantile embeddings; observed remains diagnostic.')
        ...
        bins = load_bins(args.crop, args.origin, args.condition)
        model = NeuralReadout(args.head, DIMS[args.condition], bins).cuda()
        ...
        restored = NeuralReadout(args.head, DIMS[args.condition], bins).cuda()
