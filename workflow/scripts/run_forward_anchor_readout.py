...
from forward_anchor_readout import (ROOT, RESULT, CROPS, CONDITIONS, HEADS, DIMS,
    run_root, load, NeuralReadout, predict, versions, TREATMENTS)
...
CODE = ('run_forward_anchor_readout.py', 'forward_anchor_readout.py', 'neural_process_readout.py',
...
def main():
    ...
    p.add_argument('--treatment', choices=TREATMENTS, required=True)
    ...
    root = run_root(args.crop, args.origin, args.head, args.condition, args.smoke, args.treatment)
    root.mkdir(parents=True, exist_ok=True)
    with (root / 'run.lock').open('a') as lock:
        fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
        x, arrays, meta = load(args.crop, args.origin, args.condition, args.treatment)
        spec = dict(**vars(args), seed=42, input_manifest=meta, versions=versions(),
            ...
            inputs='Matched terminal rows from 1993. Historical residuals alone are forward fitted in the forward treatment; frozen vegetation trajectories remain in-sample. Outer historical base unchanged.')
        ...
        for epoch in range((2 if args.smoke else 60)+1):
            ...
            print(f'[FORWARD READOUT] {args.crop} {args.origin} {args.head} {args.condition} {row}', flush=True)
        ...
        row.update(dimensions=DIMS[args.condition], selected_epoch=best_epoch, seed=42, treatment=args.treatment,
        ...
        print(f'[FORWARD COMPLETE] {row}', flush=True)
